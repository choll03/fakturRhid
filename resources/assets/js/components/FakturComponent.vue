    ...
    <div class="col-md-12">
      <div class="card">
        ...
        <div class="card-body">
          ...
          <v-client-table :data="fakturs" :columns="columns" :options="options">
            <template slot="actions" scope="props">
              <div class="text-center">
                <!-- <button
                  class="btn btn-sm"
                  @click="batalEdit(props.row)"
                  v-if="props.row.edit_mode == 1"
                >Batal</button>
                <button
                ...
                <a
                  ...
                  v-if="props.row.edit_mode == 0"
                >
                  <i class="fa fa-print"></i>
                </a>
                <a
                  class="btn btn-info btn-sm"
                  @click.prevent="getDetail(props.row)"
                  v-if="props.row.edit_mode == 0"
                >
              ...
              <tbody>
                <tr v-for="(detail, i) in details" :key="i">
                  ...
                  <td>
                    ...
                    {{ detail.harga_item.toLocaleString() }}
                  </td>
                  <td>
                    <span class="float-left">Rp.</span>
                ...
                <tr>
                  ...
                  <td>
                    <span class="float-left">Rp.</span>
                    <span class="float-right">{{ total.toLocaleString() }}</span>
                  </td>
                </tr>
              </tbody>
            </table>
...
export default {
  ...
  methods: {
    ...
    remove(faktur) {
      swal({
        ...
        if (result.value) {
          ...
          axios
            .delete("faktur/" + faktur.id)
            .then(res => {
              const fakturIndex = _.indexOf(this.fakturs, found);
              this.fakturs.splice(fakturIndex, 1);
              swal("Terhapus!", "Item berhasil terhapus", "success");
            })
            .catch(err => {
              swal("Oppsss!", err, "error");
            });
        }
      });
    },
    ...
    update(faktur) {
      var found = _.find(this.fakturs, ["id", faktur.id]);
      axios
        .put("faktur/" + faktur.id, faktur)
        .then(res => {
          if (typeof found == "object") {
            var index = _.indexOf(this.fakturs, found);
            this.fakturs[index].edit_mode = 0;
            swal("Sukses!", "Berhasil Merubah", "success");
          }
        })
        .catch(err => {
          swal("Oppsss!", err, "error");
